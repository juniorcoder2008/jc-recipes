...
const Form = ({ recipeList, setRecipeList }) => {
  ...
  const createRecipe = async e => {
    ...
    await axios.post('http://localhost:5000/create-recipes', {title: title, description: description, ingredients: ingredientArr}).then(res => {
      location.reload();
    });
  }
            ...
            {ingredientsArr.map(item => {
              return (
                <div className={`flex gap-2 ing-${ingredientsArr.indexOf(item)}`} key={Math.random()}>
                  <input type="text" required id="ing" placeholder='Ingredient' className='w-7/12 bg-gray-200 px-3 py-2 rounded-lg outline-none focus:text-indigo-500' />
